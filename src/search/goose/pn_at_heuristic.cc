...
PnAtHeuristic::PnAtHeuristic(
    ...
    if (!does_cache_estimates()) {
        cerr << "PnAtHeuristic needs cache_estimates=true" << endl;
...
void PnAtHeuristic::set_novelty(const State &state, int novelty) {
...
void PnAtHeuristic::get_path_dependent_evaluators(set<Evaluator *> &evals) {
    evals.insert(this);
...
vector<int> PnAtHeuristic::evaluate_state(const State &state) {
    state.unpack();
...
void PnAtHeuristic::notify_initial_state(const State &initial_state) {
    ...
    novelty_tables.emplace(eval_values, NoveltyTable(width, task_info, /*at=*/true, /*wl=*/false, nullptr));
    int novelty = novelty_tables.at(eval_values)
                      .compute_novelty_and_update_table(initial_state);
    set_novelty(initial_state, novelty);
}

void PnAtHeuristic::notify_state_transition(
    ...
    if (heuristic_cache[state].dirty) {
        ...
        if (it == novelty_tables.end()) {
            it = novelty_tables.emplace_hint(
                it, eval_values, NoveltyTable(width, task_info, /*at=*/true, /*wl=*/false, nullptr));
        }
        int novelty = -1;
...
bool PnAtHeuristic::dead_ends_are_reliable() const {
    return false;
}

int PnAtHeuristic::compute_heuristic(const State &) {
    ABORT("Novelty should already be stored in heuristic cache.");
}

class PnAtHeuristicFeature
    : public plugins::TypedFeature<Evaluator, PnAtHeuristic> {
public:
    PnAtHeuristicFeature() : TypedFeature("pnat") {
        ...
        add_heuristic_options_to_feature(*this, "pnat");
    ...
    virtual shared_ptr<PnAtHeuristic> create_component(
        ...
        return plugins::make_shared_from_arg_tuples<PnAtHeuristic>(
...
static plugins::FeaturePlugin<PnAtHeuristicFeature> _plugin;
}
