...
WlGooseHeuristic::WlGooseHeuristic(
    ...
    model = std::make_shared<features::WLFGenerator>(task, model_file);
}
...
class WlGooseHeuristicFeature
    : public plugins::TypedFeature<Evaluator, WlGooseHeuristic> {
public:
    WlGooseHeuristicFeature() : TypedFeature("wlgoose") {
        ...
        add_option<std::string>(
            "model_file", "path to trained model", "\"default_value\"");
