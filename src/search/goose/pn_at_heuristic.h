...
class PnAtHeuristic : public Heuristic {
    const int width;
...
public:
    ...
    virtual ~PnAtHeuristic() override;
...
struct OrderPnAtHeuristicsLastHack {
    bool operator()(const Evaluator *lhs, const Evaluator *rhs) const {
        if (dynamic_cast<const pn_heuristic::PnAtHeuristic *>(lhs) != nullptr &&
            dynamic_cast<const pn_heuristic::PnAtHeuristic *>(rhs) == nullptr) {
            return false;
        }
        if (dynamic_cast<const pn_heuristic::PnAtHeuristic *>(lhs) == nullptr &&
            dynamic_cast<const pn_heuristic::PnAtHeuristic *>(rhs) != nullptr) {
