...
PnWlHeuristic::PnWlHeuristic(
    ...
    if (!does_cache_estimates()) {
        cerr << "PnWlHeuristic needs cache_estimates=true" << endl;
...
void PnWlHeuristic::set_novelty(const State &state, int novelty) {
...
void PnWlHeuristic::get_path_dependent_evaluators(set<Evaluator *> &evals) {
    evals.insert(this);
...
vector<int> PnWlHeuristic::evaluate_state(const State &state) {
    state.unpack();
...
void PnWlHeuristic::notify_initial_state(const State &initial_state) {
    ...
    if (heuristic_cache[state].dirty) {
        ...
        if (it == novelty_tables.end()) {
            it = novelty_tables.emplace_hint(
                it, eval_values,
                NoveltyTable(
                    width, task_info, /*at=*/false, /*wl=*/true, nullptr));
        }
        int novelty = -1;
...
bool PnWlHeuristic::dead_ends_are_reliable() const {
    return false;
}

int PnWlHeuristic::compute_heuristic(const State &) {
    ABORT("Novelty should already be stored in heuristic cache.");
}

class PnWlHeuristicFeature
    : public plugins::TypedFeature<Evaluator, PnWlHeuristic> {
public:
    PnWlHeuristicFeature() : TypedFeature("pnwl") {
    ...
    virtual shared_ptr<PnWlHeuristic> create_component(
        ...
        return plugins::make_shared_from_arg_tuples<PnWlHeuristic>(
...
static plugins::FeaturePlugin<PnWlHeuristicFeature> _plugin;
}
